...
logger = logging.getLogger("NeuralNetworkV6")
...
class TemporalMultiScaleCNNTransformerModel(nn.Module):
    """
    Architettura neurale ibrida avanzata (v6) con blocco Convoluzionale Multi-Scale 1D
    parallelo (con kernel size 3, 5, 9 per estrarre dinamiche di brevissimo, medio e lungo termine)
    combinato con un Transformer Encoder (per le dipendenze temporali globali).
    """
    def __init__(
        self, 
        input_dim: int, 
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Pooling temporale: prendiamo l'ultimo timestep
...
class ProfitWeightedBCEWithLogitsLoss(nn.Module):
    """
    Funzione di perdita binaria (BCE) modificata per quantitativi (v6):
...
class NeuralNetworkV6(BaseModel):
    """
    Modello wrapper NeuralNetworkV6 che implementa BaseModel per l'architettura Multi-Scale CNN-Transformer
    ...
    def __init__(
        ...
        self.model = TemporalMultiScaleCNNTransformerModel(
        ...
        logger.info(f"Modello PyTorch CNN-Transformer v6 inizializzato (d_model={d_model}, nhead={nhead}, layers={num_layers}, alpha={alpha}). Device: {self.device}")
    ...
    def train(
        ...
        if HAS_TQDM and verbose:
            epoch_iterator = tqdm(epoch_iterator, desc="Progresso Totale v6", leave=True)
        ...
        for epoch in epoch_iterator:
            ...
            if HAS_TQDM and verbose:
                batch_iterator = tqdm(
                    batch_indices, 
                    desc=f"Epoca v6 {epoch:03d}/{epochs}", 
                    leave=True
                )
            else:
    ...
    def save(self, filepath: str) -> None:
        ...
        logger.info(f"Modello v6 (CNN-Transformer Multi-Scale) salvato all'indirizzo: {filepath}")

    def load(self, filepath: str) -> None:
        if not os.path.exists(filepath):
            raise FileNotFoundError(f"File dei pesi v6 non trovato all'indirizzo: {filepath}")
        ...
        if "optimizer_state_dict" in state:
            try:
                ...
                logger.warning(f"Impossibile ripristinare lo stato dell'optimizer v6: {e}.")
        
        logger.info(f"Modello v6 (CNN-Transformer Multi-Scale) caricato con successo da: {filepath}")
