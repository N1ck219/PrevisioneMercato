...
logger = logging.getLogger("NeuralNetworkV10")
...
class TemporalAttentionPooling(nn.Module):
    ...
    Invece di prendere solo l'ultimo timestep, calcola un peso di attenzione
    per ogni timestep della sequenza e restituisce la combinazione lineare ponderata.
    """
...
class TemporalMultiScaleCNNTransformerModelV10(nn.Module):
    """
    Architettura neurale ibrida avanzata V10 (CNN-Transformer + Temporal Attention Pooling).
    Combina un blocco Convoluzionale Multi-Scale 1D parallelo (con kernel size 3, 5, 9 per
    estrarre dinamiche di brevissimo, medio e lungo termine), un Transformer Encoder (per le dipendenze
    temporali globali), e un modulo di Temporal Attention Pooling con skip-connection sull'ultimo timestep
    per un'efficace e reattiva aggregazione temporale.
    """
    def __init__(
        self, 
        input_dim: int, 
        ...
        # 1. Blocco Convoluzionale Parallelo Multi-Scale (Inception-like)
        # Dividiamo i canali in uscita d_model tra i tre rami paralleli
        d_sub = d_model // 3
        ...
        # 4. Modulo di Temporal Attention Pooling
        self.attention_pooling = TemporalAttentionPooling(d_model)
        
        # 5. Testa decisionale Fully Connected
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Somma residua per ancorare la forza del giorno corrente
...
class ProfitWeightedBCEWithLogitsLoss(nn.Module):
    """
    Funzione di perdita binaria (BCE) modificata per quantitativi:
    Pondera la perdita in base alla magnitudo del rendimento futuro.
    """
...
class NeuralNetworkV10(BaseModel):
    """
    Modello wrapper NeuralNetworkV10 che implementa BaseModel per l'architettura Multi-Scale CNN-Transformer
    con Temporal Attention Pooling ed implementa l'addestramento custom con Profit-Weighted Loss function.
    """
    def __init__(
        self, 
        input_dim: int, 
        ...
        self.model = TemporalMultiScaleCNNTransformerModelV10(
        ...
        logger.info(f"Modello PyTorch CNN-Transformer v10 (Attention Pooling) inizializzato (d_model={d_model}, nhead={nhead}, layers={num_layers}, alpha={alpha}). Device: {self.device}")
    ...
    def train(
        ...
        if HAS_TQDM and verbose:
            epoch_iterator = tqdm(epoch_iterator, desc="Progresso Totale v10", leave=True)
        ...
        for epoch in epoch_iterator:
            ...
            if HAS_TQDM and verbose:
                batch_iterator = tqdm(
                    batch_indices, 
                    desc=f"Epoca v10 {epoch:03d}/{epochs}", 
                    leave=True
                )
            else:
    ...
    def save(self, filepath: str) -> None:
        ...
        logger.info(f"Modello v10 (CNN-Transformer Attention Pooling) salvato all'indirizzo: {filepath}")

    def load(self, filepath: str) -> None:
        if not os.path.exists(filepath):
            raise FileNotFoundError(f"File dei pesi v10 non trovato all'indirizzo: {filepath}")
        ...
        if "optimizer_state_dict" in state:
            try:
                ...
                logger.warning(f"Impossibile ripristinare lo stato dell'optimizer v10: {e}.")
        
        logger.info(f"Modello v10 (CNN-Transformer Attention Pooling) caricato con successo da: {filepath}")
